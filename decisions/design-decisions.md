...
**Consecuencia asumida.** Una salida inválida se descarta y admite un reintento; después la generación queda no disponible. No existe construcción determinística alternativa. Presets y plantillas son alcance opcional (`COULD`, diferidos, [DD-35](#dd-35--plantillas-y-presets-unificados-como-alcance-opcional-could)). Ver ADR 0009, D12/R-08 y D13/N-15.
...
### DD-35 · Plantillas y presets unificados como alcance opcional (COULD)

**Contexto.** El equipo resolvió no construir presets ni plantillas como funcionalidad base obligatoria de la Etapa 1 —«No usaremos preset, todo será generado desde cero con una batería de prompts», respuesta a D4 del Acta de Redefinición—. Ambos conceptos pertenecen a la misma categoría funcional (modelos de rutinas prearmadas/reutilizables).

**Opciones.** (a) Separar artificialmente la plantilla privada (RF-019) como MUST y el preset (RF-021) como COULD para forzar un piso de disponibilidad. (b) **Unificar plantillas y presets en una misma sección como alcance opcional (`COULD ⏸ DIFERIDO`), centrando la prescripción de la Etapa 1 en la generación por IA y la revisión/adaptación directa del entrenador sobre el alumno.**

**Elegida.** (b). Plantillas y presets son conceptualmente lo mismo en este proyecto (RF-019 a RF-021). No se construye un repositorio de plantillas/presets en la primera entrega.

**Consecuencia asumida.** La generación asistida por IA es la vía de creación automática. Si el servicio generativo no responde, el sistema declara la indisponibilidad temporal sin exponer errores técnicos (RF-058, RN-99) y las operaciones del resto del sistema permanecen operativas sin prometer una biblioteca de plantillas precargada.
