...
| RNF-11 | Ningún fallo generativo degrada el resto del sistema ni expone un error técnico ✎          | Prueba con API IA, ngrok o LLM inaccesible: la generación se declara no disponible y **el resto de las operaciones del sistema siguen funcionando** sin error técnico expuesto. *(Plantillas y presets son alcance opcional COULD diferido)* |
...
| RNF-18 | Las invocaciones a servicios externos de generación están limitadas por usuario y por período                                                                                                                  | Prueba: superado el límite, no se envía otra invocación y se informa indisponibilidad temporal sin afectar el resto de las operaciones del sistema ✎ |
